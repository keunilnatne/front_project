          ...
          <div className="flex flex-col gap-5">
            ...
            <section className="rounded-xl border border-[#d5d5d5] bg-white p-6">
              ...
              <div className="mt-4 flex min-h-14.5 items-center justify-between border-t border-[#eee] pt-4 text-sm">
                ...
                className="rounded-full bg-[#ffdcc3] px-2 py-1 text-xs text-[#904d00]">{isPublic ? '사내 공개' : '비공개'}</button></div>
            </section>
          </div>
          ...
          <div className="flex min-w-0 flex-col gap-5">
            <section className="rounded-xl border border-[#d5d5d5] bg-white p-6">
              <div className="flex h-8.75 items-start justify-between border-b border-[#d5d5d5]">
                <h2 className="flex items-center gap-2 text-lg font-medium"><Icon className="text-[#4338ca]">
                  <path d="M9 18h6M10 22h4M8 14a7 7 0 1 1 8 0c-1 1-1 2-1 2H9s0-1-1-2Z"/></Icon>나의 커뮤니케이션 프로파일</h2>
                <span className="rounded-md bg-[#ffdcc3] px-2 py-1 text-[11px] text-[#904d00]">
              ...
              <div className="mt-3 grid grid-cols-2 gap-2 max-md:grid-cols-1">
                ...
                className="text-xs font-medium text-[#4338ca]">{label}</p><p className="mt-1 text-sm">{value}</p></div>)}
              </div>
              ...
              <div className="mt-4 rounded-lg px-4 pt-6 pb-4">
                ...
                <div className="pt-3 text-right">
                  <button type="button" onClick={() => setIsLearningManagerOpen(true)} className="h-8 rounded-lg bg-[#4338ca] px-4 text-xs text-white shadow-sm hover:bg-[#35248f]">학습 데이터 관리 →</button></div>
              </div>
            </section>
            ...
            <section className="rounded-xl border border-[#d5d5d5] bg-white p-6">
              ...
              <div className="mt-4 grid grid-cols-3 gap-4 max-md:grid-cols-1">
                {[['평균 메시지 적합도', String(analytics.averageMessageFit), '%'],
                ['이번 달 최적화 메시지', String(analytics.optimizedMessageCount), '개'],
                ['가장 많이 협업한 직군', analytics.topCollaboratedRole || '현재 없음', '']].map(([label, value, unit]) => <div key={label}

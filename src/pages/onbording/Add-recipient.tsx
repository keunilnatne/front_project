...
function AddRecipient() {
  ...
  return (
    <main className="relative min-h-screen bg-[#f8f9fc] px-4 pb-16 pt-26.5 text-[#241912]" style={{ fontFamily: 'Pretendard Variable, Pretendard, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>
      <ProgressIndicator />

      <div className="mx-auto flex w-full max-w-120 flex-col gap-8">
        <header className="text-center">
          <h1 className="h-8 text-2xl font-semibold leading-[31.2px] tracking-[-0.24px]">자주 협업하는 사람을 추가해보세요</h1>
          <p className="mt-2 px-4 text-sm leading-5.25 text-[#564334]">협업할수록 AI가 상대방의 커뮤니케이션 방식을 더 정확하게 이해합니다.</p>
        </header>
        ...
        <form onSubmit={handleSubmit}>
          <section className="min-h-[403.19px] rounded-xl border border-[#dbdbdb] border-l-2 border-l-[#5b3df5] bg-white p-6 shadow-[0_1px_2px_rgba(0,0,0,0.05)]">
            <div className="mb-3.75 flex h-11.5 items-start gap-4 pb-1">
              <ProfileAvatar />
              <div>
                <p className="h-[17.8px] pb-[0.8px] text-xs leading-[16.8px] tracking-[0.12px] text-[#564334]">Recipient Profile</p>
                <h2 className="text-lg font-semibold leading-[25.2px]">New Connection</h2>
              </div>
            </div>
          ...
          <div className="pt-8 text-center">
            <button type="submit" className="flex h-12 w-full items-center justify-center gap-2 rounded-lg bg-[linear-gradient(90deg,#5b3df5_0%,#35248f_100%)] text-xs leading-[16.8px] tracking-[0.12px] text-white shadow-[0_1px_2px_rgba(0,0,0,0.05)] transition hover:brightness-110 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#5b3df5]/20">
              추가하기 <span aria-hidden="true" className="text-lg leading-none">→</span>
            </button>
            <button type="button" onClick={skipRecipient} className="mt-4 h-[17.8px] text-xs leading-[16.8px] text-[#564334] transition hover:text-[#35248f] focus-visible:outline-none focus-visible:underline">나중에 추가하기</button>
          </div>
        </form>
      </div>
    </main>

...
function isRecipient(value: unknown): value is Recipient {
  ...
  return typeof item.id === 'number'
    ...
    && (item.responseSpeed === '빠름' || item.responseSpeed === '보통' || item.responseSpeed === '느림')
    && typeof item.averageResponseMinutes === 'number'
    && (item.collaborationActivity === 'High' || item.collaborationActivity === 'Medium' || item.collaborationActivity === 'Low')
